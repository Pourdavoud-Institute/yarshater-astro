...
const youtubeThumbnailURL = computed(() => {
    return getYoutubeThumbnail(props.video.data.url);
});

const eventTitle = computed(() => {
    return getVideoEventTitle(props.video);
});

const speakersList = computed(() => {
    return getVideoSpeakersList(props.video);
});
</script>

<template>
    <article class="video-preview__list">
        <div class="text-wrapper | flow" data-tempo="allegro">
            <h2 class="title">
                <a :href="`/media${video.data.slug}`" class="title-link">{{
                    video.data.title
                }}</a>
            </h2>
        ...
        <a :href="`/media${video.data.slug}`" class="image-wrapper">
            <div class="overlay">
                <svg
                    aria-hidden="true"
